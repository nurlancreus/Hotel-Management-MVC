...
namespace HotelManagement.ViewComponents.Default
{
    public class _HomePageFeaturedRooms : ViewComponent
    {
        ...
        {
            Room presidentalRoom = _roomManager.TGetList().LastOrDefault(a => a.RoomType == "Presidental");

            var StandartRooms = _roomManager.TGetList().Where(a => a.RoomType == "Standart").OrderByDescending(a => a.Id).Take(2).ToList();
